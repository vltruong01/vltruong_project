...
<body>
  <main>
    <section class="profile" aria-labelledby="profile-title">
      ...
      <p class="intro">Đây là infor của mình ^^</p>
      ...
      <footer>© 2025 Vương Lộc Trường · All rights reserved</footer>
    </section>
  </main>
  ...
  <div id="chatbotModal" class="modal" role="dialog" aria-modal="true" aria-labelledby="chatbotTitle" onclick="backdropClose(event, 'chatbotModal')">
    <div class="modal-card">
      ...
      <h2 id="chatbotTitle">⏳ Vui lòng chờ chatbot đánh răng</h2>
      <p id="status" class="modal-text">Nó vừa mới ngủ dậy 🥱</p>
      ...
      <div class="modal-actions"><button class="small-btn" type="button" onclick="closeModal('chatbotModal')">Đóng</button></div>
    </div>
  </div>
  ...
  <script>
    ...
    function openChatbot() {
      ...
      chatbotTimer = setInterval(() => {
        ...
        bar.style.width = Math.min(100, Math.round((1 - sec / total) * 100)) + "%";
      ...
      chatbotPoll = setInterval(() => {
        ...
        fetch(CHATBOT_URL + "health?t=" + Date.now(), { cache: "no-store" })
          .then((response) => {
            if (response.ok && !didRedirect) {
              didRedirect = true;
              clearInterval(chatbotTimer);
              clearInterval(chatbotPoll);
              chatbotTimer = null;
              chatbotPoll = null;
              status.textContent = "✅ Chatbot đã sẵn sàng!";
              bar.style.width = "100%";
